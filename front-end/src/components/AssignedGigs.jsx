...
function AssignedGigs() {
    // component displays all the gigs assigned to admin here admin can see the teachers that have applied for the gig and select a suitable teacher
    const [assignedGigs,setassignedGigs] = useState([])
    const [search, setSearch] = useState("");
    const [displayedGIGs, setDisplayedGIGs] = useState(assignedGigs);
    const [loading, setLoading] = useState(true);
    const [reload,setReload] = useState(false);

    const fuse = new Fuse(assignedGigs, {
                ...
                <div className="flex-1 overflow-hidden">
                    <ul className="mt-4 space-y-6 max-h-[calc(100vh-250px)] overflow-y-auto">
                        {displayedGIGs.map((each_gig) => (
                            <AdminGigCard key={each_gig.gigId} gig={each_gig} isAtAssigned={true} getReload={setReload}/>
                        ))}
                    </ul>
                </div>
                </>
